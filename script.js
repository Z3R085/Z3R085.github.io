...
var start = function () {
  ...
  room.loadImpulse(
    "https://s3-us-west-2.amazonaws.com/s.cdpn.io/131247/factory.hall.ogg"
  );
        ...
        if (position >= bars) {
          {
            // time display
            p.textContent =
              ("000" + (Math.floor(bars) + 1)).substr(-3) +
              "." +
          ...
          if (0 < mod4) {
            closedHat(
              startTime,
              Math.pow(Math.sin((mod4 / 4.0) * Math.PI), 10.0) * 0.3
            );
          }
          if (4 == mod32) {
            playSynth(
              startTime,
              semiquaverTime * 2.0,
              3000 - Math.cos((position / 24) * Math.PI * 2.0) * 1000.0,
              240,
              0.1,
              chordA
            );
          ...
          if (30 == mod32) {
            playSynth(
              startTime,
              semiquaverTime * 2.0,
              3000 - Math.cos((position / 28) * Math.PI * 2.0) * 2000.0,
              240,
              0.2,
              chordC
            );
          ...
          synthPanning.pan.linearRampToValueAtTime(
            Math.sin(((index % 7) / 7.0) * Math.PI * 2.0) * 0.5,
            startTime
          );
        }
